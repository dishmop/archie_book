...
def create_2x2_figure():
    ...
    base_scene = dict(
        ...
        aspectratio=dict(x=1, y=1, z=0.4),
        camera=dict(
            eye=dict(x=0.9, y=0.9, z=0.6),
            up=dict(x=0, y=0, z=1)
        )
    )

    fig.update_layout(
        scene=dict(**base_scene, domain=dict(x=[0, 0.5], y=[0.54, 1])),
        scene2=dict(**base_scene, domain=dict(x=[0.5, 1], y=[0.54, 1])),
        scene3=dict(**base_scene, domain=dict(x=[0, 0.5], y=[0.04, 0.5])),
        scene4=dict(**base_scene, domain=dict(x=[0.5, 1], y=[0.04, 0.5])),
        paper_bgcolor='white',
        height=800,
        width=900,
        margin=dict(l=0, r=0, t=0, b=0),
        # Labels at bottom of each subplot
        annotations=[
            dict(text="n=1, k=0", x=0.20, y=0.51, xref="paper", yref="paper",
                 showarrow=False, font=dict(size=28)),
            dict(text="n=2, k=0", x=0.82, y=0.51, xref="paper", yref="paper",
                 showarrow=False, font=dict(size=28)),
            dict(text="n=1, k=1", x=0.20, y=0.01, xref="paper", yref="paper",
                 showarrow=False, font=dict(size=28)),
            dict(text="n=2, k=1", x=0.82, y=0.01, xref="paper", yref="paper",
                 showarrow=False, font=dict(size=28)),
        ]
    )
